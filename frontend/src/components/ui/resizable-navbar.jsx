...
export const Navbar = ({ children, className, isPreview = false }) => {
  ...
  return (
    <motion.div
      ref={ref}
      className={cn(`sticky inset-x-0 ${isPreview ? 'top-20' : 'top-5'} z-40 w-full`, className)}
    >
...
//       {items?.sectionVisibility && Object.entries(items.sectionVisibility).map(([section, isVisible]) =>
//         isVisible && (
//           <a
//             key={section}
//             href={`#${section}`}
...
export function NavItems({ items, onItemClick }) {
  const [hovered, setHovered] = useState(null);

  // Get only visible sections
  const visibleSections = items?.sectionVisibility
    ? Object.entries(items.sectionVisibility).filter(([_, isVisible]) => isVisible)
    : [];
        ...
        {visibleSections.map(([section], idx) => (
          <a
            ...
            onClick={(e) => {
              ...
              if (element) {
                const yOffset = -80; // 👈 jitna padding chahiye (navbar height ya extra gap)
            ...
            className="relative px-4 py-2 text-neutral-600 dark:text-neutral-300 capitalize"
          >
